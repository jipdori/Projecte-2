...
public class Client {
    ...
    public ArrayList<Client> ordenarReserves() throws SQLException {
        ...
        try {
            ...
            System.out.println("\nMOSTRANT TOTS ELS CLIENTS...");
            String query = "select c.*,c.nie from clients c JOIN realitzen r ON c.nie=r.nie JOIN activitats a ON  r.id_activitat=a.id_activitat GROUP BY c.nie ORDER BY count(c.nie) DESC";
            PreparedStatement ps = con.prepareStatement(query);
            ResultSet rs = ps.executeQuery(query);
